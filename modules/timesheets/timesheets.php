...
function timesheets_load_js() {
	...
	if ($data_timekeeping_form == 'timekeeping_manually') {
		echo '<script src="' . base_url('modules/timesheets/assets/js/check_in_out_ts.js') . '?v=1009' . TIMESHEETS_REVISION . '"></script>';
...
function after_render_top_search_timesheets() {
	...
	if ((has_permission('attendance_management', '', 'view_own') || has_permission('attendance_management', '', 'view') || is_admin())) {
		...
		$has_checkin = $CI->timesheets_model->has_check_in(get_staff_user_id(), date('Y-m-d'));
		if ($data_timekeeping_form == 'timekeeping_manually') {
			echo '<li class="dropdown">
					<a href="#" class="check_in_out_timesheet" data-toggle="tooltip" title="" onclick="open_check_in_out();" data-placement="bottom" data-original-title="' . _l('check_in') . ' / ' . _l('check_out') . '">';
			echo '<i class="fa fa-history fa-fw fa-lg ';
			echo $has_checkin ? ' green' : '';
			echo '"></i>
