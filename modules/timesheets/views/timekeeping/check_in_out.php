      <?php 
        ...
        $has_checkout = $CI->timesheets_model->has_check_out(get_staff_user_id(), date('Y-m-d'));
        $count_check_outs = $CI->timesheets_model->count_check_outs(get_staff_user_id(), date('Y-m-d'));
      ?>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              ...
              <h4>
               <?php echo _l('check_in'); ?> / <?php echo _l('check_out'); ?> (Số lần chấm công: <?php echo $count_check_outs; ?>)
             </h4>
           </div>
            ...
            <div class="row">
              <div class="col-md-12">
                ...
                <div class="col-md-12 bottom_co_btn">
                  <div class="bottom_co_btn_item text-center">
                    <?php
                    if(($type_check_in_out == '' || $type_check_in_out == 2 || $allows_updating_check_in_time == 1)){ ?>
                      <div>
                        <button class="btn d-inline" id="toggle-camera">Lật Camera</button>
                      </div><br>
                      <?php echo form_open(admin_url('timesheets/check_in_ts'),array('id'=>'timesheets-form-check-in', 'onsubmit'=>'get_data(event)', 'enctype'=>"multipart/form-data")); ?>
                      <div class="text-center">
                        <div>
                          <video id="video" src="" style="max-height: 300px; max-width: 100%; margin-bottom:30px" autoplay playsinline muted></video>
                        </div>
                      </div>
                      ...
                      <input id="imageFile" type="file" name="img_data" style="display:none;">
                      <input type="hidden" name="location_user" value="">
                      <button class="btn btn-primary check_in"><?php echo _l('check_in'); ?></button>
                     <?php echo form_close(); } ?>
                   </div>
                   <div class="bottom_co_btn_item text-center">              
                     <?php if(($type_check_in_out == 1 || $allows_updating_check_in_time == 1)){ ?>
                      <div>
                          <button class="btn" id="toggle-camera">Lật Camera</button>
                        </div><br>
                      <?php echo form_open(admin_url('timesheets/check_in_ts'),array('id'=>'timesheets-form-check-out', 'onsubmit'=>'get_data(event)', 'enctype'=>"multipart/form-data")); ?>  
                      <div class="text-center">
                        <div>
                          <video id="video" src="" style="max-height: 300px; max-width: 100%; margin-bottom:30px" autoplay playsinline muted></video>
                        </div>
