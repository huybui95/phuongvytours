 ...
 */
	...
	public function check_in($data) {
		...
		if (isset($_FILES['img_data'])) {
			$targetDirectory = get_upload_path_by_type('timesheets');
    		$targetFile = uniqid() . '.png';
			if (move_uploaded_file($_FILES['img_data']['tmp_name'], $targetDirectory . $targetFile)) {
				$data['img_data'] = $targetFile;
			}
		}
		if ($data['edit_date'] != '') {
 ...
 */
	...
	function has_check_out($staff, $date) {
		$data_check_in_out = $this->db->query('select id from ' . db_prefix() . 'check_in_out where staff_id = ' . $staff . ' and date(' . db_prefix() . 'check_in_out.date) = "' . $date . '" and type_check = 2 order by id desc limit 1')->row();
		if ($data_check_in_out) {
			return true;
		}
		return false;
	}
	function count_check_ins($staff, $date) {
		$result = $this->db->query(
			'SELECT COUNT(*) AS check_in_count FROM ' . db_prefix() . 'check_in_out 
			 WHERE staff_id = ' . $staff . ' 
			 AND DATE(' . db_prefix() . 'check_in_out.date) = "' . $date . '" 
			 AND type_check = 1'
		)->row();
	
		if ($result) {
			return (int) $result->check_in_count;
		}
		return 0;
	}
	function count_check_outs($staff, $date) {
		$result = $this->db->query(
			'SELECT COUNT(*) AS check_out_count FROM ' . db_prefix() . 'check_in_out 
			 WHERE staff_id = ' . $staff . ' 
			 AND DATE(' . db_prefix() . 'check_in_out.date) = "' . $date . '" 
			 AND type_check = 2'
		)->row();
		...
		if ($result) {
			return (int) $result->check_out_count;
		}
		return 0;
	}
    ...
    public function get_data_attendance_export_2($month_filter, $department_filter, $role_filter, $staff_filter)
    {
        ...
        return $this->get_attendance_data($staffs, $g_month, $year);
    }
	
    public function get_attendance_data($staffs, $g_month, $year) {
    try {
        $db_prefix = db_prefix();

        $this->db->select("
            cio.staff_id AS staffid,
            MAX(s.firstname) AS firstname,
            MAX(s.lastname) AS lastname,
            MAX(s.email) AS email,
            MAX(s.staff_identifi) AS staff_identifi,
            MAX(s.sex) AS sex,
            MAX(s.job_position) AS job_position,
            DATE(MIN(cio.date)) AS work_date,
            DAYNAME(DATE(MIN(cio.date))) AS day_of_week,
            MAX(j.position_name) AS position_name,
            MAX(j.position_code) AS position_code,
            GROUP_CONCAT(DISTINCT d.name SEPARATOR ', ') AS department_names,
            GROUP_CONCAT(
                CASE 
                    WHEN cio.type_check = 1 THEN TIME(cio.date)
                    ELSE NULL 
                END 
                ORDER BY cio.date SEPARATOR ','
            ) AS check_ins,
            GROUP_CONCAT(
                CASE 
                    WHEN cio.type_check = 2 THEN TIME(cio.date)
                    ELSE NULL 
                END 
                ORDER BY cio.date SEPARATOR ','
            ) AS check_outs
        ...
        $this->db->from("{$db_prefix}check_in_out AS cio");
        $this->db->join("{$db_prefix}staff AS s", "cio.staff_id = s.staffid", 'left');
        $this->db->join("{$db_prefix}hr_job_position AS j", "s.job_position = j.position_id", 'left');
        $this->db->join("{$db_prefix}staff_departments AS sd", "s.staffid = sd.staffid", 'left');
        $this->db->join("{$db_prefix}departments AS d", "sd.departmentid = d.departmentid", 'left');

        $staff_ids = array_column($staffs, 'staffid');
        if (!empty($staff_ids)) {
            $this->db->where_in("cio.staff_id", $staff_ids);
        }

        if (!empty($g_month) && !empty($year)) {
            $this->db->where("MONTH(cio.date)", $g_month);
            $this->db->where("YEAR(cio.date)", $year);
        }

        $this->db->group_by(["cio.staff_id", "DATE(cio.date)"]);
        $this->db->order_by("cio.staff_id, work_date");
