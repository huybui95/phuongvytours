...
class timesheets extends AdminController {
	...
	public function timekeeping() {
		...
		$data['roles'] = $this->db->query('select * from '.db_prefix().'hr_job_position')->result_array();
	...
	public function reload_timesheets_byfilter() {
		...
		if ($staff != '') {
			...
			if ($data['job_position'] != '') {
				$staff_querystring .= 'job_position = ' . $data['job_position'];
			} else {
			...
			if ($access_history) {
				foreach ($access_history as $key => $value) {
					if ($value['type_check'] == '1') {
						$access_history_string .= '<li class="list-group-item"><i class="fa fa-sign-in text-success" aria-hidden="true"></i> ' . _dt($value['date']) . ($value['img_data'] ? ' (<a href="' . site_url('uploads/timesheets/' . $value['img_data']) . '" target="_blank">Hình ảnh</a>)' : '') . ($value['location_user'] ? ' <a href="https://www.google.com/maps?q=' . $value['location_user'] . '" target="_blank">(Vị trí)</a>' : '') . ($value['ip_address'] ? ' <a href="https://ipinfo.io/' . $value['ip_address'] . '" target="_blank">(IP Address)</a>' : '') . '</li>';
					} else {
						$access_history_string .= '<li class="list-group-item"><i class="fa fa-sign-out text-danger" aria-hidden="true"></i> ' . _dt($value['date']) . ($value['location_user'] ? ' <a href="https://www.google.com/maps?q=' . $value['location_user'] . '" target="_blank">(Vị trí)</a>' : '') . ($value['ip_address'] ? ' <a href="https://ipinfo.io/' . $value['ip_address'] . '" target="_blank">(IP Address)</a>' : '') . '</li>';
					}
				}
			}
...
public function check_in_ts() {
	if ($this->input->post()) {
		...
		// $count_check_outs = $this->timesheets_model->count_check_outs($data['staff_id'], $current_date);
		// if($count_check_outs >= 4) {
		// 	set_alert('danger', 'Bạn đã chấm công đủ số lần trong ngày hôm nay');
		// 	redirect(admin_url('/'));
		// } else {
		...
		if ($this->input->post()) {
			...
			foreach ($data_check_in_out as $key => $value) {
				...
				if ($value['type_check'] == 2) {
					// Cảm ơn những đóng góp của bạn hôm nay. Ra lúc
					$type_check = _l('checked_out_at');
					$alert_type = 'alert-warning';
					$html_list .= '<div class="row"><div class="col-md-12"><div class="alert '.$alert_type.'">Cảm ơn những đóng góp của bạn hôm nay. Ra lúc '._dt($value['date']).'</div></div></div>';
				} else {
					$html_list .= '<div class="row"><div class="col-md-12"><div class="alert ' . $alert_type . '">Bạn đã bắt đầu công việc ' . _dt($value['date']) . ' Chúc 1 ngày làm việc hiệu quả</div></div></div>';
				}
			}
			echo json_encode([
				'html_list' => $html_list,
 ...
 */
	...
	public function export_attendance_excel() {
		...
		if ($this->input->post()) {
			...
			$list = $this->timesheets_model->get_data_attendance_export_2($month_filter, $department_filter, $role_filter, $staff_filter);
			$month = date('m');
			$month_year = date('Y');
            ...
            // Định dạng tiêu đề
            $header_format = ['font' => 'Calibri', 'font-size' => 18, 'font-style' => 'bold', 'halign' => 'center'];
            $subheader_format = ['font' => 'Calibri', 'font-size' => 13, 'font-style' => 'bold', 'halign' => 'left'];

            // Chuyển đổi tháng filter sang ngày bắt đầu và kết thúc
            $start_date = date('01/m/Y', strtotime($month_filter . "-01"));
            $end_date = date('t/m/Y', strtotime($month_filter . "-01"));

            // Lấy tên phòng ban từ ID
            $department_name = $this->get_department_name_by_id($department_filter);

            // Thêm các hàng tiêu đề vào Excel
            $writer->writeSheetRow('Sheet1', ["BÁO CÁO NHẬT KÝ CHẤM CÔNG"], $header_format);
            $writer->markMergedCell('Sheet1', 0, 0, 0, 13);
            $writer->writeSheetRow('Sheet1', ["Từ ngày: $start_date đến ngày: $end_date"], $subheader_format);
            $writer->writeSheetRow('Sheet1', ["Phòng ban: $department_name"], $subheader_format);
            $writer->writeSheetRow('Sheet1', []); // Dòng trống
            ...
            $header = [
                ...
                'Ngày'          => 'string',
                'Thứ'           => 'string',
                'Chấm vào lần 1'=> 'string',
                'Chấm ra lần 1' => 'string',
                'Chấm vào lần 2'=> 'string',
                'Chấm ra lần 2' => 'string',
                'Tổng giờ làm'  => 'string',
            ];

            $writer->writeSheetRow('Sheet1', array_keys($header), $col_options = ['fill' => '##4E81BD', 'font-style' => 'bold', 'color' => '#FFFFFF', 'border' => 'left,right,top,bottom', 'height' => 25, 'border-color' => '#FFFFFF', 'font-size' => 13, 'font' => 'Calibri']);
            ...
            $stt = 1;
            foreach ($list as $row) {
                // Chuyển check-in/check-out thành mảng
                $check_ins = explode(',', $row['check_ins']);
                $check_outs = explode(',', $row['check_outs']);

                // Ghép dữ liệu vào các cột
                $row_data = [
                    $stt++,
                    $row['staff_identifi'],
                    ...
                    $check_ins[1] ?? '',
                    $check_outs[1] ?? '',
                    $this->calculate_total_hours($check_ins, $check_outs),
                ];
			...
			$filename = "attendance_Report_{$month_filter}.xlsx";
    ...
    private function get_department_name_by_id($department_id) {
        $CI =& get_instance();
        $CI->db->select('name');
        $CI->db->from(db_prefix() . 'departments');
        $CI->db->where('departmentid', $department_id);
        $result = $CI->db->get()->row();
        return $result ? $result->name : 'Tất cả phòng ban';
    }

    private function calculate_total_hours($check_ins, $check_outs) {
        $total_seconds = 0;

        for ($i = 0; $i < min(count($check_ins), count($check_outs)); $i++) {
            if (!empty($check_ins[$i]) && !empty($check_outs[$i])) {
                $start = strtotime($check_ins[$i]);
                $end = strtotime($check_outs[$i]);
                $total_seconds += ($end - $start);
            }
        }

        return gmdate("H:i", $total_seconds); // Trả về dạng HH:MM
    }
