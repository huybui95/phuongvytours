...
class Invoice_supplier_items extends AdminController
{
    ...
    {
        parent::__construct();
        $this->load->model('invoice_supplier_items_model');
    }
    ...
    public function index()
    {
        ...
        $data['items_groups'] = $this->invoice_supplier_items_model->get_groups();
        ...
        $data['title'] = _l('invoice_supplier_items');
        $this->load->view('admin/invoice_supplier_items/manage', $data);
    }
    ...
    public function table()
    {
        ...
        $this->app->get_table_data('invoice_supplier_items');
    }
        ...
        if (staff_can('view',  'items')) {
            if ($this->input->post()) {
                ...
                if ($data['itemid'] == '') {
                    ...
                    $id      = $this->invoice_supplier_items_model->add($data);
                    $success = false;
                    $message = '';
                    if ($id) {
                        $success = true;
                    ...
                    echo json_encode([
                        'success' => $success,
                        'message' => $message,
                        'item'    => $this->invoice_supplier_items_model->get($id),
                    ]);
                } else {
                    ...
                    $success = $this->invoice_supplier_items_model->edit($data);
                    $message = '';
                    if ($success) {
    ...
    public function import()
    {
        ...
        $this->load->view('admin/invoice_supplier_items/import', $data);
    }

    public function add_group()
    {
        if ($this->input->post() && staff_can('create',  'items')) {
            $this->invoice_supplier_items_model->add_group($this->input->post());
    ...
    public function update_group($id)
    {
        if ($this->input->post() && staff_can('edit',  'items')) {
            $this->invoice_supplier_items_model->edit_group($this->input->post(), $id);
    ...
    public function delete_group($id)
    {
        if (staff_can('delete',  'items')) {
            if ($this->invoice_supplier_items_model->delete_group($id)) {
                set_alert('success', _l('deleted', _l('item_group')));
            }
        }
        redirect(admin_url('invoice_supplier_items?groups_modal=true'));
    }
    ...
    public function delete($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('invoice_supplier_items'));
        }

        $response = $this->invoice_supplier_items_model->delete($id);
                ...
                foreach ($ids as $id) {
                    if ($this->input->post('mass_delete')) {
                        if ($has_permission_delete) {
                            if ($this->invoice_supplier_items_model->delete($id)) {
                                $total_deleted++;
                            }
                        }
                    }
    ...
    public function search()
    {
        if ($this->input->post() && $this->input->is_ajax_request()) {
            echo json_encode($this->invoice_supplier_items_model->search($this->input->post('q')));
        }
    }
    ...
    public function copy($id)
    {
        ...
        $data = (array) $this->invoice_supplier_items_model->get($id);

        $id = $this->invoice_supplier_items_model->copy($data);

        if ($id) {
            set_alert('success', _l('item_copy_success'));
            return redirect(admin_url('invoice_supplier_items?id=' . $id));
        }

        set_alert('warning', _l('item_copy_fail'));
        return redirect(admin_url('invoice_supplier_items'));
    }
}
