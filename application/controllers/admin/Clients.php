...
class Clients extends AdminController
{
    ...
    public function check_phone_unique() {
        if ($this->input->is_ajax_request()) {
            ...
            $existing_contact = $this->db->get_where('contacts', array('phonenumber' => $phone_number))->row();
    
            if ($existing_contact && $existing_contact->id != $contact_id) {
                // Phone number exists and belongs to a different contact (for update scenario)
                echo json_encode(array('status' => 'error'));
            } else {
                // Phone number is unique or belongs to the same contact (for update scenario)
                echo json_encode(array('status' => 'success'));
            }
        } else {
            show_404(); // Handle non-AJAX requests appropriately
        }
    }
    public function check_duplicate_customer_phone_number()
    {
        if (has_permission('customers', '', 'create')) {
            $cccd = trim($this->input->post('phonenumber'));
            $response = [
                'exists' => (bool) total_rows(db_prefix() . 'clients', ['phonenumber' => $phonenumber]) > 0,
                'message' => _l('phonenumber_exists_info', '<b>' . $cccd . '</b>'),
            ];
            echo json_encode($response);
        }
    }
