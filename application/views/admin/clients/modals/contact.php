            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="form-group" app-field-wrapper="phonenumber"><small class="req text-danger">* </small><label for="phonenumber" class="control-label"><?= _l('client_phonenumber'); ?></label><input type="text" id="phonenumber" name="phonenumber" class="form-control" autocomplete="off" required="1" value="<?php echo $value; ?>"></div>
...
<script>
    ...
    $('#phonenumber').on('input', function() {
        ...
        var contactId = '<?php echo isset($contact) ? $contact->id : ''; ?>';

        // Perform an AJAX request to check if the phone number is unique
        $.ajax({
            type: 'POST',
            url: '<?php echo admin_url('clients/check_phone_unique'); ?>',
            data: {
            ...
            success: function(response) {
                if (response.status === 'error') {
                    $('#phonenumber-error').text('Số điện thoại đã tồn tại!');
    ...
    // Initially disable the submit button if the phone number is empty
    $('#contact-form button[type="submit"]').prop('disabled', true);

    // Enable or disable the submit button based on the initial state of the phone number field
