...
return App_table::find('clients')
    ->outputUsing(function ($params) {
        ...
        $aColumns = [
            ...
            db_prefix() . 'clients.expected_time_on_tour as expected_time_on_tour', // Thêm cột Expected Time On Tour
            db_prefix() . 'clients.active',
            db_prefix() . 'clients.address as address', // Thêm cột Address
            ...
            db_prefix() . 'clients.datecreated as datecreated', // Giữ nguyên cột Ngày tạo
        ];
    ...
    <label class="onoffswitch-label" for="' . $aRow['userid'] . '"></label>
            ...
            // Expected Time On Tour
            $row[] = e($aRow['expected_time_on_tour'] ?? ''); // Thêm dữ liệu cho cột Expected Time On Tour
            // For exporting
            $toggleActive .= '<span class="hide">' . ($aRow[db_prefix() . 'clients.active'] == 1 ? _l('is_active_export') : _l('is_not_active_export')) . '</span>';

            $row[] = $toggleActive;
            // Address
            $row[] = e($aRow['address'] ?? ''); // Thêm dữ liệu cho cột Address
            ...
            // Date created
            $row[] = e(_dt($aRow['datecreated'])); // Giữ nguyên cột Ngày tạo
