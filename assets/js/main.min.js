$(window).on("load", (function () {
 ...
 function elFinderBrowser(field_name, url, type, win) {
   window.elfinderCallback = function(fileUrl) {
       win.document.getElementById(field_name).value = fileUrl;
       win.document.getElementById(field_name).dispatchEvent(new Event('change'));
   };
   ...
   tinymce.activeEditor.windowManager.open({
       ...
       width: 900,
       height: 500,
       file: admin_url + 'utilities/media',
       inline: 1, // Bắt buộc với TinyMCE v4
       resizable: 'yes',
       popup_css: false
   }, {
       window: win,
...
$("body").on("loaded.bs.select change", "select.ajax-search", (function (e) {
 ...
 function lead_mark_as(e, t) {
    ...
    a.status = e, a.leadid = t, $.post(admin_url + "leads/update_lead_status", a).done((function (e) {
       ...
       !0 !== a.success && "true" != a.success || setTimeout((() => {
          window.location.href = a.redirect + "#init-lead-" + t
       }), 500), table_leads.DataTable().ajax.reload(null, !1)
    }))
 }
 ...
 function clear_item_preview_supplier_values(e) {
    var t = $("table.items tbody").find("tr:last-child").find("select").selectpicker("val"),
        a = $(".main");
    a.find("textarea").val(""),
    ...
    a.find('input[name="link_image"]').val(""), // Clear link_image input
...
//  function get_item_preview_values_supplier() {
//     var e = {};
...
//     // Lấy giá trị base64 từ input ẩn trong .main
//     e.base64data = $('.main input[name="base64data"]').val() || "data:image/png;base64,";
//     return e;
// }
 ...
 function get_item_preview_values_supplier() {
    ...
    e.rate = $('.main input[name="rate"]').val();
    e.unit = $('.main input[name="unit"]').val();
    e.link_image = $('.main input[name="link_image"]').val();
    return e;
}

function add_item_to_table_supplier(e, t, a, i) {
    if (typeof a !== 'undefined' && a !== '') {
        try {
            e = $.extend({}, get_item_preview_values_supplier(), JSON.parse(a));
        } catch (error) {
            console.error('Error parsing new_item:', error);
        }
    }
    if ('' !== (e = void 0 === e || 'undefined' == e ? get_item_preview_values_supplier() : e).description || '' !== e.long_description || '' !== e.rate) {
        var n = '',
            s = lastAddedItemKey ? (lastAddedItemKey += 1) : $('body').find('tbody .item').length + 1;
        lastAddedItemKey = s;
        ...
        return get_taxes_dropdown_template(l, e.taxname).done(function(a) {
            ...
            l.length > 0 && $.each(l, function() {
                ...
                if (a.is(':checkbox')) {
                    ...
                    $.each(l, function(e, t) {
                        ...
                        if ('1' == $(this).attr('data-custom-field-required')) r = true;
                    });
                    $.each(o, function(t, a) {
                        e.find('input[value="' + $(a).val() + '"]').attr('checked', true);
                    });
                    t = e.html();
                    ...
                    if ('1' == e.find('[data-fieldid]').attr('data-custom-field-required')) r = true;
                    t = e.html();
                } else if (a.is('select')) {
                    if ('1' == $(this).attr('data-custom-field-required')) r = true;
                    var d = $(this).find('select[data-fieldid]').selectpicker('val');
                    ...
                    p.append(_);
                    $.each(d, function(e, t) {
                        p.find('select option[value="' + t + '"]').attr('selected', true);
                    });
                    t = p.html();
                }
                n += '<td class="custom_field">' + t + '</td>';
            ...
            // Image column
            n += '<td>';
            n += '<div class="image_file">';
            n += '<span for="imageUpload' + s + '" id="imagePreview' + s + '">';
            // Nếu có link_image thì hiển thị ảnh, nếu không thì để trống
            if (e.link_image && e.link_image !== '') {
                n += '<img src="' + e.link_image + '" alt="" height="100" width="100%">';
            } else {
                n += '<span>No image</span>'; // Placeholder khi không có ảnh
            }
            n += '</span>';
            n += '</div>';
            n += '<div class="drag-area">';
            // Input hidden luôn có, nhưng value chỉ set nếu có link_image
            n += '<input type="hidden" name="newitems[' + s + '][link_image]" id="link_image' + s + '" value="' + (e.link_image || '') + '">';
            n += '<div class="row">';
            n += '<div class="col-md-12" style="text-align:center;">';
            n += '<div class="ajax-upload-dragdrop">';
            n += '<div class="upload" style="position: relative; overflow: hidden; cursor: pointer;">';
            n += '<i class="fa fa-camera" aria-hidden="true"></i>';
            n += '<input type="file" id="imageUpload' + s + '" name="newitems[' + s + '][profile_boss_photo]" class="imageUpload form-control" style="position: absolute; cursor: pointer; top: 0px; width: 100%; height: 34px; left: 0px; z-index: 100; opacity: 0;" aria-invalid="false">';
            n += '</div>';
            n += '</div>';
            n += '</div>';
            n += '</div>';
            n += '</td>';
            n += '<td class="amount" align="right">' + format_money(o, true) + '</td>';
            ...
            // Update validator for dynamic fields
            var validator = $('#invoice-form').data('validator');
            if (validator) {
                $('#imageUpload' + s).rules('add', {
                    accept: 'image/*'
                });
                $('#link_image' + s).rules('add', {
                    required: false
                });
                $('textarea[name="newitems[' + s + '][description]"]').rules('add', {
                    required: true
                });
                $('input[name="newitems[' + s + '][qty]"]').rules('add', {
                    required: true,
                    number: true
                });
                $('input[name="newitems[' + s + '][rate]"]').rules('add', {
                    required: true,
                    number: true
                });
                validator.elements();
            ...
            init_datepicker();
            init_color_pickers();
            clear_item_preview_supplier_values();
            reorder_items();
            $('body').find('#items-warning').remove();
            $('body').find('.dt-loader').remove();
            $('#item_select').selectpicker('val', '');
            if (r && $('.invoice-form').length) {
                validate_invoice_form();
            } else if (r && $('.estimate-form').length) {
                validate_estimate_form();
            } else if (r && $('.proposal-form').length) {
                validate_proposal_form();
            } else if (r && $('.credit-note-form').length) {
                validate_credit_note_form();
            }
            if ('undefined' != i && i) {
                $('select[name="task_select"]').find('[value="' + c + '"]').remove();
 ...
 $("body").on("loaded.bs.select change", "select.ajax-search", (function (e) {
    ...
    if (e.startsWith("#init-lead-")) {
       init_lead(e.slice(11))
    }
 })), $(document).on("click", ".convert_lead_to_customer_b2c", (function () {
